...
/**
 * One-screen control panel: start/stop the translation server, show its
 * status, toggle start-on-boot, and request exclusion from battery
 * optimization (important on Boox e-ink devices, which kill background apps
 * eagerly).
 */
public class MainActivity extends Activity {

    public static final String PREFS = "translator";
    ...
    private void refresh() {
        ...
        sb.append(getString(R.string.status_prefix)).append(TranslatorService.status);
        if (TranslatorService.running) {
            sb.append("\n\nKOReader server URL: http://127.0.0.1:")
              .append(TranslatorService.PORT)
              .append("\n(Japanese → Local translation server)");
        }
